...
class ShaderTest extends FunSuite:
  ...
  out.color = in.color;"""
    ...
    assert(wgsl.contains("struct VertexInput {"), s"Missing VertexInput struct:\n$wgsl")
    assert(wgsl.contains("@location(0) position: vec3<f32>"), s"Missing position attribute:\n$wgsl")
    assert(wgsl.contains("@location(1) color: vec4<f32>"), s"Missing color attribute:\n$wgsl")

    // Check VertexOutput struct (varyings + builtin position)
    assert(wgsl.contains("struct VertexOutput {"), s"Missing VertexOutput struct:\n$wgsl")
    assert(wgsl.contains("@location(0) color: vec4<f32>"), s"Missing varying color:\n$wgsl")
    assert(wgsl.contains("@builtin(position) position: vec4<f32>"), s"Missing builtin position:\n$wgsl")

    // Check FragmentOutput struct
    assert(wgsl.contains("struct FragmentOutput {"), s"Missing FragmentOutput struct:\n$wgsl")

    // Check vertex main
    assert(wgsl.contains("@vertex"), s"Missing @vertex annotation:\n$wgsl")
    assert(wgsl.contains("fn vs_main(in: VertexInput) -> VertexOutput"), s"Missing vs_main signature:\n$wgsl")
    assert(wgsl.contains("var out: VertexOutput"), s"Missing out variable in vertex:\n$wgsl")
    assert(wgsl.contains("return out;"), s"Missing return out in vertex:\n$wgsl")

    // Check fragment main
    assert(wgsl.contains("@fragment"), s"Missing @fragment annotation:\n$wgsl")
    assert(wgsl.contains("fn fs_main(in: VertexOutput) -> FragmentOutput"), s"Missing fs_main signature:\n$wgsl")
    assert(wgsl.contains("var out: FragmentOutput"), s"Missing out variable in fragment:\n$wgsl")
  }
  ...
  test("shader with custom vertex builtin input") {
    ...
    assert(wgsl.contains("@builtin(vertex_index) vertexIndex: u32"),
  ...
  out.color = in.color;"""
    ...
    assert(vertexInputIdx < vertexOutputIdx, "VertexInput should come before VertexOutput")
    assert(vertexOutputIdx < fragmentOutputIdx, "VertexOutput should come before FragmentOutput")
    assert(fragmentOutputIdx < uniformIdx, "Structs should come before uniforms")
